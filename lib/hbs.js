...
ExpressHbs.prototype.cachePartials = function(cb) {
  var self = this;
  var fileFilter = '*.*';
  // whether to check for partials prefixed with underscore, eg _partial.hbs
  var isUnderscored = !this.partialsDir;
  if (isUnderscored) {
    this.partialsDir = [this.viewsDir];
    fileFilter = '_*.*';
  }
  ...
  function readNext() {
    readdirp({ root: self.partialsDir[count], fileFilter: fileFilter })
    ...
    .on('data', function(entry) {
      ...
      var basename = path.basename(entry.name, path.extname(entry.name));
      if (isUnderscored) basename = basename.slice(1);

      var name = dirname + basename;
...
function _express3(filename, source, options, cb) {
  ...
  this.loadDefaultLayout(options.cache, function(err) {
    ...
    if (!options.cache || !self.isPartialCachingComplete) {
